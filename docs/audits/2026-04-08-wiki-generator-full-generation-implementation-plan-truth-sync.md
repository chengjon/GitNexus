# Wiki Generator Full Generation Implementation Plan Truth Sync

日期：2026-04-08
范围：`docs/superpowers/plans/2026-03-28-wiki-generator-full-generation-implementation-plan.md`
目标：把历史 `wiki-generator-full-generation` implementation plan 的 false-open 状态同步回当前已落地事实。

---

## 1. 背景

`full-generation` 这条 wiki generator 拆分切片，当前问题不在实现，而在历史计划与设计
叙事：

- 历史 implementation plan 仍把整条执行链保留成未勾选状态
- 历史设计文档仍写成 `Draft for review`
- 但当前仓内已经存在 `full-generation.ts`、
  `wiki-full-generation.test.ts`，以及 `generator.ts` 对它的实际导入锚点
- `wiki-full-generation.test.ts` 已经覆盖 no-source-files error、
  enriched file list、leaf/parent dispatch、existing-page short-circuit、
  metadata save、failure accumulation 与 core progress window 契约
- `2026-03-28` technical-debt audit 还明确记录了
  `full-generation.ts` → `wiki-full-generation.test.ts` 已验证，并指出早期
  `failedModules` review issue 已在落地代码中修正

因此它属于典型的 historical false-open plan debt，而不是 full-generation
module 缺失。
...
- [2026-03-28-wiki-generator-full-generation-design.md](/opt/claude/GitNexus/docs/superpowers/specs/2026-03-28-wiki-generator-full-generation-design.md)
...
- [2026-03-28-technical-debt-audit.md](/opt/claude/GitNexus/docs/superpowers/specs/2026-03-28-technical-debt-audit.md)
  - 已记录 full-generation extraction 的验证状态与 review issue 收敛事实
...
- 把旧计划中的 `failedModules` 叙事同步到当前已落地实现：
  helper 内部聚合失败列表，wrapper 再 merge 回实例状态
...
- `openspec validate 2026-04-08-wiki-generator-full-generation-implementation-plan-truth-sync`
  返回 `Change '2026-04-08-wiki-generator-full-generation-implementation-plan-truth-sync' is valid`
- `gitnexus_detect_changes({scope: "all", repo: "GitNexus", cwd: "/opt/claude/GitNexus"})`
  直接返回：
  - `risk_level = critical`
  - `changed_files = 102`
...
- 它不等于本轮 truth-sync 自身引入了新的 wiki full-generation blast radius
...
修完后，`full-generation` 这条已经落地的 wiki generator 拆分切片，
