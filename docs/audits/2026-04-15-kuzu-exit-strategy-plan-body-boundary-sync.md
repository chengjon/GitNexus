# Kuzu Exit Strategy Plan Body Boundary Sync

日期：2026-04-15  
范围：`docs/superpowers/plans/2026-04-06-kuzu-dependency-exit-strategy-implementation-plan.md`  
目标：给 `kuzu` exit-strategy implementation plan 的旧正文补更显式的 boundary 说明，避免保留的 `Goal`、`Architecture` 和已勾选计划步骤被继续读成当前 live dependency-governance plan

---

## 1. 背景

[`2026-04-06-kuzu-dependency-exit-strategy-implementation-plan.md`](/opt/claude/GitNexus/docs/superpowers/plans/2026-04-06-kuzu-dependency-exit-strategy-implementation-plan.md)
...
如果没有这层 boundary note，读者仍可能直接把这份计划页读成当前 live dependency-governance plan，而不是已经被后续 audit / OpenSpec 吸收的历史计划。

因此 residual 不在 `kuzu` exit-strategy 结论本身，而在 historical implementation-plan 正文边界还不够显式。
...
- [2026-04-06-kuzu-dependency-exit-strategy.md](/opt/claude/GitNexus/docs/audits/2026-04-06-kuzu-dependency-exit-strategy.md)
  - 提供同一 `kuzu` 线的 authoritative follow-up baseline
...
- 在 audit 与 OpenSpec 中登记这条 `kuzu` exit-strategy plan boundary sync
...
- 不重写 `kuzu` exit-strategy implementation plan 内容
- 不重开依赖治理工作
...
openspec validate 2026-04-15-kuzu-exit-strategy-plan-body-boundary-sync
...
- `openspec validate 2026-04-15-kuzu-exit-strategy-plan-body-boundary-sync`
  - 返回 `Change '2026-04-15-kuzu-exit-strategy-plan-body-boundary-sync' is valid`
- `gitnexus_detect_changes({scope: "staged", repo: "GitNexus", cwd: "/opt/claude/GitNexus"})`
  - 直接返回：
    - `risk_level = low`
    - `changed_files = 5`
...
- staged scope review 只看到本轮预期的 `kuzu` exit-strategy historical-plan boundary 切片
...
这轮关闭的是 `kuzu` exit-strategy implementation plan 旧正文边界不够显式的问题，
不是新的依赖治理任务。
