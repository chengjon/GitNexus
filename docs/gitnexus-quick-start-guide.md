...
如果你正在修改当前这个 GitNexus 仓库本身，而不只是把它用在别的项目里，顶层开发治理规则在 [`../DEVELOPMENT_RULES.md`](../DEVELOPMENT_RULES.md)。
...
主入口请先看 [README.md](../README.md)。本文是中文快速说明；如果你需要
本机 AI CLI / local agent 命令版，请看
[ai-cli-local-quick-start.md](./ai-cli-local-quick-start.md)。如果你要核对当前
状态、历史收敛或专题审计，请看 [audits/README.md](./audits/README.md)。
...
```bash
npx gitnexus analyze --with-context
```

这会额外生成：
- `CLAUDE.md` 和 `AGENTS.md` 上下文文件
- 项目级技能目录 `.claude/skills/gitnexus/`
...
`detect_changes` 相关提醒：
- 多仓会话中显式传 `repo`
- 如果当前 worktree 与 MCP 服务器 cwd 不一致，再显式传 `cwd`
...
如需在当前 GitNexus 仓库自身的治理语境下理解优先级：

- **主支持面**：Claude Code、Codex
- **可选集成面**：Cursor / Windsurf 等其他 MCP host
- **差异说明**：Claude Code 与 Codex 都属于主支持面；当前差异主要是
  配置/自动化体验差异，而不是支持层级差异
...
| `detect_changes` | 提交前检查变更影响范围 | `detect_changes({scope: "staged", repo: "my-repo"})` |
...
4. 提交前运行 `detect_changes({scope: "all", repo: "my-repo"})` 验证
5. 如果在 worktree 中，或 MCP 服务器 cwd 不是当前仓库，再额外传 `cwd`
...
| **提交前必须运行 `detect_changes`** | 确认变更范围符合预期；多仓传 `repo`，worktree/cwd 不一致时再传 `cwd` |
...
> **Codex 用户**：当前没有等价自动钩子；如需最新索引，请在这些变更后手动重跑 `npx gitnexus analyze`。
