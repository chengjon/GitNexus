...
- `docs/superpowers/specs/2026-03-28-wiki-generator-full-generation-review.md`
  的旧 review 正文边界说明也已继续收敛：
  - 现在在 `Verdict` / `Summary` 入口前显式注明这些段落属于
    2026-03-28 design-review baseline
  - 避免读者继续把保留的 blocker/severity 叙事读成当前 implementation gate
  - 该结论已登记为：`openspec/changes/2026-04-15-full-generation-review-body-boundary-sync/`
- upstream doc/governance convergence first-pass baseline 的旧正文边界说明也已继续收敛：
  - `docs/audits/2026-04-06-upstream-doc-governance-convergence-baseline.md`
    现在在 `Refresh Summary` / `Recommended Replay Order` 入口前显式注明这些段落属于
    2026-04-06 first refreshed-fetch baseline
  - 避免读者继续把旧 same-day replay counts、inventory 与顺序建议直接读成当前 live checklist
  - 该结论已登记为：`openspec/changes/2026-04-15-upstream-doc-governance-baseline-body-boundary-sync/`
- `docs/audits/2026-04-06-kuzu-dependency-review.md` 的旧 review-only 正文边界说明也已继续收敛：
  - 现在在顶部、`Provisional Recommendation` 与 `Immediate Operating Rule` 入口前显式注明这些段落属于
    2026-04-06 dependency-governance review baseline
  - 避免读者继续把旧 recommendation / next-step 语气直接读成当前 live package policy
  - 该结论已登记为：`openspec/changes/2026-04-15-kuzu-dependency-review-body-boundary-sync/`
- `docs/audits/2026-04-06-kuzu-dependency-exit-strategy.md` 的旧 follow-up 正文边界说明也已继续收敛：
  - 现在在顶部、`Exit Criteria` 与 `Current Decision` 入口前显式注明这些段落属于
    2026-04-06 tracked-exception baseline
  - 避免读者继续把旧 exit criteria / reopen-trigger 语气直接读成当前唯一的 live package policy
  - 该结论已登记为：`openspec/changes/2026-04-15-kuzu-exit-strategy-body-boundary-sync/`
