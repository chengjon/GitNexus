...
- `src/core/wiki/generator.ts` 的 worktree 拆分仍在进行，但 `full-generation` review 中提出的 `failedModules` 双通道问题已在本地 worktree 修复并通过针对性测试
  - review 文档已提交：`77732ce` (`docs(review): add wiki generator review notes`)
  - follow-up fix 已提交：`7595f7d` (`fix(wiki): internalize full generation failures`)
  - 当前 worktree 已 clean，剩余工作是 review / merge 而不是继续补代码
...
1. 审阅并合并 `src/core/wiki/generator.ts` worktree 中已完成的 follow-up commits
...
- `docs/audits/2026-04-06-repo-technical-debt-and-residual-audit.md` 已建立为仓库卫生治理基线入口；后续 OpenSpec 收口按独立治理切片推进
  - 其中 `detect_changes` 宿主 Finding 3 的后续状态同步已补做：`docs/audits/2026-04-08-repo-technical-debt-audit-status-sync.md`

仍待完成：

- 合并 `src/core/wiki/generator.ts` 当前 worktree 切片（代码与 review follow-up 已提交）
- 审阅并按需要推送 `src/core/kuzu/kuzu-adapter.ts` 当前 FTS 抽取 commit
- 审阅并按需要推送 `src/cli/analyze.ts` 当前 helper 抽取 commit
- 按观察名单规则评估 `src/core/kuzu/kuzu-adapter.ts`、`src/cli/skill-gen.ts`、`src/core/ingestion/utils.ts`、`src/core/ingestion/import-processor.ts`、`src/core/ingestion/framework-detection.ts` 是否再次成为主动切片目标
