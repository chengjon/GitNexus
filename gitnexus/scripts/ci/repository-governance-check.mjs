...
const REQUIRED_PR_METRIC_LABELS = [
  'Measured:',
  'Inferred:',
  'Historical Baseline:',
];

const EMPTY_METRIC_VALUE_PATTERNS = [
  /^\s*(?:n\/a|na|none)\s*$/i,
];
...
const DEVELOPMENT_RULES_ENTRYPOINT_EXEMPT_PATHS = new Set([
  'CHANGELOG.md',
...
const COMPATIBILITY_PATH_PATTERNS = [
  /shim/i,
  /compat/i,
  /_v2\.[^/]+$/i,
  /_new\.[^/]+$/i,
...
const REQUIRED_COMPATIBILITY_FILE_MARKERS = [
  {
    ...
    message: 'Compatibility-like files added in managed paths must declare `CANONICAL PATH:` in the file body.',
  },
  {
    rule: 'compat-file-direct-cutover-risk',
    pattern: /DIRECT CUTOVER RISK:/i,
    message: 'Compatibility-like files added in managed paths must declare `DIRECT CUTOVER RISK:` in the file body.',
  },
  {
    rule: 'compat-file-exit-condition',
    pattern: /EXIT CONDITION:/i,
    message: 'Compatibility-like files added in managed paths must declare `EXIT CONDITION:` in the file body.',
  },
  {
    rule: 'compat-file-cleanup-tracking',
    pattern: /CLEANUP TRACKING:/i,
    message: 'Compatibility-like files added in managed paths must declare `CLEANUP TRACKING:` in the file body.',
  },
];
...
function hasMarkdownEntrypointChecklist(section) {
  ...
  return /developer-facing markdown entrypoint/i.test(content) &&
...
function tokenizePathLikeValues(value) {
  return String(value ?? '')
    ...
    .filter((token) => token.includes('/') || token.includes('.') || token.includes('_') || token.includes('-'));
}
...
function touchesCompatibilityLikePath(entry) {
  return getEntryPaths(entry).some((filePath) => isManagedPath(filePath) && isCompatibilityLikePath(filePath));
}
...
function hasTemporaryScriptLikeFilename(filePath) {
  const baseName = path.posix.basename(filePath);
  return SCRIPT_FILE_EXTENSION_PATTERN.test(baseName) &&
...
function isTemporaryMigrationScriptLikePath(filePath) {
  const baseName = path.posix.basename(filePath);
  return isScriptDirectoryPath(filePath) &&
...
export function findPullRequestBodyViolations(body, changedFiles = []) {
  ...
  const compatibilityRelevant = relevantChanges.some((entry) => touchesCompatibilityLikePath(entry));
  ...
  const activeCompatibilityPathRelevant = relevantChanges.some((entry) =>
    !entry.status.startsWith('D') &&
    entry.path.length > 0 &&
    isManagedPath(entry.path) &&
    isCompatibilityLikePath(entry.path),
  );
  const activeTemporaryScriptRelevant = relevantChanges.some((entry) =>
    !entry.status.startsWith('D') &&
    entry.path.length > 0 &&
    isManagedPath(entry.path) &&
    isTemporaryScriptLikePath(entry.path),
  );
  const activeTemporaryMigrationScriptRelevant = relevantChanges.some((entry) =>
    !entry.status.startsWith('D') &&
    entry.path.length > 0 &&
    isManagedPath(entry.path) &&
  ...
  const activeCleanupTrackingRelevant = activeCompatibilityPathRelevant || activeTemporaryScriptRelevant;
  ...
  if (compatibilityRelevant && hasValue(compatibilityValue)) {
    ...
    if (!hasPathReference(compatibilityValue, compatibilityTargets)) {
      violations.push({
        ...
        message: 'Compatibility Layer / Shim must name the actual shim, compat, *_new, or *_v2 path being changed.',
      });
    }

    if (hasPathReference(compatibilityValue, compatibilityTargets) && !hasRepoRelativePathReference(compatibilityValue, compatibilityTargets)) {
      violations.push({
        rule: 'pr-compatibility-path-format',
        message: 'Compatibility Layer / Shim must name the actual repo-relative shim or compat path, such as `gitnexus/src/router_new.ts`.',
      });
    }
  }
  ...
  if (compatibilityRelevant && directCutoverRiskFieldPresent && !hasValue(directCutoverRiskValue)) {
    violations.push({
      ...
      message: 'Direct Cutover Risk must explain why direct cutover is unsafe right now when compatibility-layer paths change.',
    });
  }

  if (compatibilityRelevant && hasValue(directCutoverRiskValue) && isPlaceholderDirectCutoverRisk(directCutoverRiskValue)) {
    violations.push({
      rule: 'pr-direct-cutover-risk-quality',
      message: 'Direct Cutover Risk must explain the concrete blocker for direct cutover and cannot be a placeholder such as TBD, later, unknown, or keep for safety.',
    });
  }
  ...
  if (activeExitConditionRelevant && !hasValue(exitConditionValue)) {
    violations.push({
      ...
      message: 'Exit Condition must be filled when compatibility-layer paths change or a temporary migration/debug script remains active after the change.',
    });
  }

  if (activeExitConditionRelevant && hasValue(exitConditionValue) && isPlaceholderExitCondition(exitConditionValue)) {
    violations.push({
      rule: 'pr-exit-condition-quality',
      message: 'Exit Condition must describe a concrete retirement trigger for the compatibility path or temporary script and cannot be a placeholder such as TBD, later, or follow-up cleanup.',
    });
  }
  ...
  if (
    ...
    (canonicalPathReferencesCompatibilityLayer(canonicalPathValue) || canonicalPathReferencesTemporaryScript(canonicalPathValue))
  ) {
    violations.push({
      rule: 'pr-canonical-path-compatibility',
      message: 'Canonical Path must point to the stable canonical path, not the shim, compat, *_new, *_v2, or temporary migration/debug script path.',
    });
  }

  if (activeTemporaryScriptRelevant && hasValue(canonicalPathValue) && canonicalPathReferencesTemporaryScript(canonicalPathValue)) {
    violations.push({
      rule: 'pr-canonical-path-temporary-script',
      message: 'Canonical Path must point to the stable product path, not the temporary migration/debug script or debug entry point.',
    });
  }
  ...
  if (relevantChanges.length > 0 && hasValue(canonicalPathValue) && !canonicalPathHasRepoRelativeTarget(canonicalPathValue)) {
    violations.push({
      rule: 'pr-canonical-path-format',
      message: 'Canonical Path must name exactly one stable repo-relative path such as `gitnexus/src/router.ts`.',
    });
  }
  ...
  if ((migrationRelevant || activeTemporaryMigrationScriptRelevant) && !migrationStatusFieldPresent) {
    violations.push({
      rule: 'pr-migration-status-field',
      message: 'Migration Status field must be present when compatibility layers, managed-path retirement, or active temporary migration scripts are involved.',
    });
  }

  if ((migrationRelevant || activeTemporaryMigrationScriptRelevant) && migrationStatusFieldPresent && !hasValue(migrationStatusValue)) {
    violations.push({
      rule: 'pr-migration-status',
      message: 'Migration Status must be filled when compatibility layers, managed-path retirement, or active temporary migration scripts are involved.',
    });
  }

  if ((migrationRelevant || activeTemporaryMigrationScriptRelevant) && hasValue(migrationStatusValue) && !migrationStatusKind) {
    violations.push({
      rule: 'pr-migration-status-format',
      message: 'Migration Status must start with one of: transitional, partial, in progress, complete, or N/A.',
    });
  }

  if (activeCompatibilityPathRelevant && migrationStatusKind && migrationStatusKind !== 'transitional') {
    violations.push({
      rule: 'pr-migration-status-active-compat',
      message: 'Migration Status must stay transitional or partial while an active shim, compat, *_new, or *_v2 path still exists after the change.',
    });
  }

  if (activeTemporaryMigrationScriptRelevant && migrationStatusKind && migrationStatusKind !== 'transitional') {
    violations.push({
      rule: 'pr-migration-status-active-temp-migration',
      message: 'Migration Status must stay transitional or partial while an active temporary migration/backfill/cutover script still exists after the change.',
    });
  }
  ...
  if (activeCleanupTrackingRelevant && !cleanupTrackingFieldPresent) {
    violations.push({
      ...
      message: 'Cleanup Tracking field must be present when an active temporary layer or temporary script remains after the change.',
    });
  }

  if (activeCleanupTrackingRelevant && cleanupTrackingFieldPresent && !hasValue(cleanupTrackingValue)) {
    violations.push({
      rule: 'pr-cleanup-tracking',
      message: 'Cleanup Tracking must name the milestone, issue, or follow-up task that removes the remaining temporary layer or script.',
    });
  }
  ...
  if (
    ...
    violations.push({
      ...
      message: 'Cleanup Tracking must name a concrete milestone, issue, or follow-up task reference such as `GNX-123`, `milestone 4`, or `issue ...`, and cannot be a placeholder such as TBD, later, N/A, or same PR.',
    });
  }
  ...
  if (deletionRelevant && !hasValue(deletionReachabilityValue)) {
    violations.push({
      ...
      message: 'Deletion Reachability must be filled when managed files are deleted or renamed away in managed paths.',
    });
  }

  if (deletionRelevant && hasValue(deletionReachabilityValue) && !hasDeletionReachabilityCoverage(deletionReachabilityValue)) {
    violations.push({
      rule: 'pr-deletion-reachability-coverage',
      message: 'Deletion Reachability must explicitly cover feature-tree, runtime, scripts/automation, config/env, and tests/fixtures reachability.',
    });
  }
  ...
  if (deletionRelevant && !hasValue(gitnexusEvidenceValue)) {
    violations.push({
      ...
      message: 'GitNexus Evidence must be filled when managed files are deleted or renamed away in managed paths.',
    });
  }

  if (deletionRelevant && hasValue(gitnexusEvidenceValue) && !hasGitNexusEvidence(gitnexusEvidenceValue)) {
    violations.push({
      rule: 'pr-deletion-evidence-format',
      message: 'GitNexus Evidence must cite a GitNexus tool call or gitnexus:// resource when managed files are deleted or renamed away in managed paths.',
    });
  }
  ...
  if (deletionRelevant && hasValue(gitnexusEvidenceValue) && hasGitNexusEvidence(gitnexusEvidenceValue)) {
    ...
    if (!hasPathReference(gitnexusEvidenceValue, evidenceTargets)) {
      violations.push({
        ...
        message: 'GitNexus Evidence must name the retired managed path or the canonical replacement path.',
      });
    }

    if (hasPathReference(gitnexusEvidenceValue, evidenceTargets) && !hasRepoRelativePathReference(gitnexusEvidenceValue, evidenceTargets)) {
      violations.push({
        rule: 'pr-deletion-evidence-target-format',
        message: 'GitNexus Evidence must name the retired managed path or canonical replacement path as a repo-relative path such as `gitnexus/src/router.ts`.',
      });
    }
  }
  ...
  if (
    ...
    violations.push({
      ...
      message: 'Metrics Claims must include at least one filled metric field or an explicit N/A note.',
    });
  }
  ...
  if (
    ...
    violations.push({
      ...
      message: 'Metrics Claims `N/A:` must stay empty whenever Measured, Inferred, or Historical Baseline is filled.',
    });
  }
  ...
  if (/mechanical split/i.test(content)) {
    ...
    const hasMechanicalSplitBenefit = MECHANICAL_SPLIT_BENEFIT_PATTERNS.some((pattern) => pattern.test(mechanicalSplitContext));
    if (!hasMechanicalSplitBenefit) {
      violations.push({
        rule: 'pr-mechanical-split-benefit',
        message: 'Mechanical split notes must explain a concrete benefit such as ownership boundaries, call-path clarity, testability, or change safety.',
      });
    }
  }
...
export async function findCompatibilityMetadataViolations(
  ...
  for (const entry of relevantChanges) {
    ...
    if (hasValue(directCutoverRiskValue) && isPlaceholderDirectCutoverRisk(directCutoverRiskValue)) {
      violations.push({
...
export async function findTemporaryScriptMetadataViolations(
  ...
  for (const entry of relevantChanges) {
    ...
    if (!hasValue(canonicalPathValue)) {
      ...
      if (canonicalPathReferencesCompatibilityLayer(canonicalPathValue) || canonicalPathReferencesTemporaryScript(canonicalPathValue)) {
        violations.push({
...
async function runPullRequestMode(eventPath, repoRoot, baseRef) {
  ...
  const violations = [...bodyViolations, ...compatibilityFileViolations, ...temporaryScriptViolations];
  if (violations.length > 0) {
    printViolations(violations);
    process.exitCode = 1;
    return;
...
async function main() {
  ...
  throw new Error('usage: --mode <paths|pr-body> [--repo-root <path>] [--base-ref <ref>] [--event-path <path>]');
}
