...
export class LocalBackend {
  ...
  private async _impactImpl(repo: RepoHandle, params: ImpactParams): Promise<any> {
    ...
    // contract after target resolution, before traversal.
    ...
    if (autoExpanded.length > 0) {
      return {
        ...
        relation_types_hint:
          `Impact auto-included ${autoExpanded.join(', ')} because the target is a ${symType}. ` +
  ...
  private async apiImpact(
    ...
    try {
      ...
      if (!params.route && !params.file) {
        return {
          ...
          recovery: {
            ...
            steps: [
              'Use route_map() to see all available routes',
              'Try api_impact({ route: "..." }) with a route name',
              'Try api_impact({ file: "..." }) with a file path',
            ],
          },
          index_status: buildIndexStatus(repo),
        };
      ...
      if (routes.length === 0) {
        ...
        return {
          error: `No routes found matching "${target}"${verb}.`,
          next_steps: [
            'Use route_map() to see all available routes',
      ...
      // Count verbs per handler from the FULL match (before the method filter)
