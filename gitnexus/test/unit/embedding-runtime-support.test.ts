...
let tempGitNexusHome: string | undefined;
...
beforeEach(() => {
  ...
  tempGitNexusHome = fs.mkdtempSync(path.join(os.tmpdir(), 'gitnexus-embedding-runtime-'));
  process.env.GITNEXUS_HOME = tempGitNexusHome;
});

afterEach(() => {
  vi.unstubAllGlobals();
  const homeToRemove = tempGitNexusHome;
  tempGitNexusHome = undefined;
  for (const key of EMBED_ENV_KEYS) {
    if (savedEnv[key] === undefined) delete process.env[key];
    else process.env[key] = savedEnv[key];
  }
  if (homeToRemove) fs.rmSync(homeToRemove, { recursive: true, force: true });
});
