...
describe('repository governance path checks', () => {
  ...
  it('does not treat migration-style *_new files as generic temp-file violations', () => {
    const violations = findManagedPathViolations(['gitnexus/src/router_new.ts']);
    ...
    expect(violations).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          path: 'gitnexus/scripts/debug-cutover-trace.mjs',
        }),
        expect.objectContaining({
  ...
  it('path mode requires developer-facing markdown entrypoints to reference DEVELOPMENT_RULES.md', async () => {
    const violations = await findPathModeViolations(
      ['README.md', 'docs/gitnexus-quick-start-guide.md'],
    ...
    expect(violations).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          path: 'docs/gitnexus-quick-start-guide.md',
        }),
      ]),
    );
  });
...
`);
    ...
    expect(violations.find((entry) => entry.rule === 'pr-governance-fields')?.message).toContain(
      'Compatibility Layer / Shim',
    );
    expect(violations.find((entry) => entry.rule === 'pr-governance-fields')?.message).toContain(
      'Deletion Reachability',
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-workline-lane-format' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-validation-content' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-validation-na-exclusive' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-direct-cutover-risk' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-direct-cutover-risk-quality' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-cleanup-tracking' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-cleanup-tracking-quality' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-exit-condition' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-exit-condition-quality' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-compatibility-target' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-compatibility-path-format' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-canonical-path-multiple' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-deletion-evidence-target' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-deletion-evidence-format' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-canonical-path' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-canonical-path-format' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-metrics-content' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-metrics-na-exclusive' })]),
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-metric-context' })]),
    );
    expect(violations.find((entry) => entry.rule === 'pr-metric-context')?.message).toContain(
      'Measured',
    );
  });
...
`,
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'pr-mechanical-split-benefit' })]),
    );
  });
...
describe('repository governance temporary script metadata checks', () => {
  ...
  it('rejects placeholder purpose metadata for newly added temporary scripts', async () => {
    ...
    expect(violations).toEqual(
      expect.arrayContaining([expect.objectContaining({ rule: 'temp-script-purpose-quality' })]),
    );
  });
  ...
  it('rejects newly added temporary scripts whose filename is only a generic temporary label', async () => {
    ...
    expect(violations).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          rule: 'temp-script-name-specificity',
          path: 'gitnexus-web/scripts/migration-cutover.sh',
        }),
      ]),
    );
  });
