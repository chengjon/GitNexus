...
- [x] 1.1 Add a dedicated OpenSpec change for structured gpu-device-node output
- [x] 1.2 Bound the slice to `gpu-device-node` transport only

## 2. Test-First Coverage

- [x] 2.1 Add focused doctor regression coverage for visible-node gpu-device-node data
- [x] 2.2 Add focused doctor regression coverage for missing-node gpu-device-node data
- [x] 2.3 Run the focused test and confirm it fails before implementation

## 3. Structured Output Convergence

- [x] 3.1 Add structured `data` to `gpu-device-node` checks
- [x] 3.2 Reuse existing platform and visible-node results
