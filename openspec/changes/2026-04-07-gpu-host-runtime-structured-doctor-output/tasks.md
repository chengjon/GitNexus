...
- [x] 1.1 Add a dedicated OpenSpec change for structured gpu-host-runtime output
- [x] 1.2 Bound the slice to `gpu-host-runtime` transport only

## 2. Test-First Coverage

- [x] 2.1 Add focused doctor regression coverage for successful gpu-host-runtime data
- [x] 2.2 Add focused doctor regression coverage for `ENOENT` gpu-host-runtime data
- [x] 2.3 Run the focused test and confirm it fails before implementation

## 3. Structured Output Convergence

- [x] 3.1 Add structured `data` to `gpu-host-runtime` checks
- [x] 3.2 Reuse existing command result metadata and summary
