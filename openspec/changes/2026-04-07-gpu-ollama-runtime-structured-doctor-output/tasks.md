...
- [x] 1.1 Add a dedicated OpenSpec change for structured gpu-ollama-runtime output
- [x] 1.2 Bound the slice to `gpu-ollama-runtime` transport only

## 2. Test-First Coverage

- [x] 2.1 Add focused doctor regression coverage for GPU-offload pass gpu-ollama-runtime data
- [x] 2.2 Add focused doctor regression coverage for CPU-fallback fail gpu-ollama-runtime data
- [x] 2.3 Run the focused test and confirm it fails before implementation

## 3. Structured Output Convergence

- [x] 3.1 Add structured `data` to `gpu-ollama-runtime` checks
- [x] 3.2 Reuse existing provider/probe/runtime results
