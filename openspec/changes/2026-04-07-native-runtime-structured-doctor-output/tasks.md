...
- [x] 1.1 Add a dedicated OpenSpec change for structured native-runtime doctor output
- [x] 1.2 Bound the slice to additive `data` for the default native-runtime check

## 2. Test-First Coverage

- [x] 2.1 Add focused doctor regression coverage for the default native-runtime snapshot path
- [x] 2.2 Run the focused test and confirm it fails before implementation

## 3. Structured Output Convergence

- [x] 3.1 Add `data` to the default `native-runtime` check
- [x] 3.2 Reuse `NativeRuntimeSnapshot` directly as the structured payload
