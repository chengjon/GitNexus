...
- [x] 1.1 Add a dedicated OpenSpec change for `gitnexus-impact-analysis`
      detect-changes metadata convergence
- [x] 1.2 Bound the slice to the two impact-analysis skill-doc surfaces, audit,
      roadmap update, and OpenSpec record

## 2. Truth Source

- [x] 2.1 Re-read the current source impact-analysis skill wording
- [x] 2.2 Re-read the current package impact-analysis skill wording
- [x] 2.3 Reconfirm the historical skills-review note for metadata guidance
- [x] 2.4 Reconfirm the current `detect_changes` metadata contract

## 3. Skill-Doc Convergence

- [x] 3.1 Expand the workflow guidance so `detect_changes` path verification is
      explicit
- [x] 3.2 Add the metadata fields to the `detect_changes` example
- [x] 3.3 Add checklist guidance that requires checking `git_diff_path`,
      `process_cwd`, `path_resolution`, and `fallback_reason`
- [x] 3.4 Keep source skill and package skill wording aligned
...
- `openspec validate 2026-04-08-gitnexus-impact-analysis-detect-changes-metadata-convergence`
  returned `Change '2026-04-08-gitnexus-impact-analysis-detect-changes-metadata-convergence' is valid`
